...
public class MainActivity extends CheckPermissionsActivity {
    ...
    private String time;
    private ArrayList<String> hum_list; //湿度
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //加载背景图片
        Glide.with(this).load(R.drawable.bj).into(mbg);
        //获取日期
    ...
    //获取今天的空气质量（精确到市）
    private void get_air(String xy) {
        //使用Get异步请求
        OkHttpClient client = new OkHttpClient();
        Request request = new Request.Builder()
                //拼接访问地址
                .url("https://devapi.qweather.com/v7/air/now?key=6a6eb9aa868342bba2bbb3f79a4df3ac&location=" + xy)
                .build();
            ...
            public void onResponse(Call call, Response response) throws IOException {
                if(response.isSuccessful()){
                    ...
                    try {
                        ...
                        JSONObject object = result_json.getJSONObject("now");

                        try {
                            JSONObject jb = object;
                            aqi = jb.getString("aqi");
                            pm10 = jb.getString("pm10");
                            pm25 = jb.getString("pm2p5");
                ...
                MainActivity.this.runOnUiThread(new Runnable() { //在子线程更新UI线程
                    @Override
                    public void run() {
                        mpm10.setText(pm10);
                        mpm25.setText(pm25);
    ...
    //获取今天的天气和温度
    private void get_weather(String xy) {
        //使用Get异步请求
        OkHttpClient client = new OkHttpClient();
        Request request = new Request.Builder()
                //拼接访问地址
                .url("https://devapi.qweather.com/v7/weather/now?key=6a6eb9aa868342bba2bbb3f79a4df3ac&location=" + xy)
                .build();
        client.newCall(request).enqueue(new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                if(response.isSuccessful()){//回调的方法执行在子线程。
                    ...
                    try {
                        ...
                        JSONObject object2 = result_json2.getJSONObject("now");

                        // 天气和温度

                        JSONObject json3 = object2;
                        weather = json3.getString("text");
                        temp = json3.getString("temp");
                        // 更新时间
                        String up_time = object2.getString("obsTime");
    ...
    //获取一周温湿度数据
    private void get_forecast(String xy) {
        //使用Get异步请求
        OkHttpClient client = new OkHttpClient();
        Request request = new Request.Builder()
                //拼接访问地址
                .url("https://devapi.qweather.com/v7/weather/7d?key=6a6eb9aa868342bba2bbb3f79a4df3ac&location=" + xy)
                .build();
        client.newCall(request).enqueue(new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
...
//                        JSONArray jsonArray3 = result_json3.getJSONArray("HeWeather6");
                        JSONObject object3 = JSON.parseObject(response.body().string());
                        JSONArray forecast = object3.getJSONArray("daily");
//                        System.out.println((forecast));
                        ...
                        for(int i=0;i<forecast.size();i++){
                            ...
                            hum_list.add(hum.get("humidity").toString());
                            max_temp_list.add(hum.get("tempMax").toString());
                            min_temp_list.add(hum.get("tempMin").toString());
                            date_list.add(hum.get("fxDate").toString().substring(6)); // 截取月日
                        }
                    } catch (JSONException e) {
                        e.printStackTrace();
                    }
    ...
    private void startOneLocaton() {
        ...
        locationClientOption.setScanSpan(0); // 定位时间间隔
    ...
    public BDAbstractLocationListener oneLocationListener = new BDAbstractLocationListener() {
        ...
        public void onReceiveLocation(BDLocation location) {
            ...
            LatLng latLng = new LatLng(location.getLatitude(), location.getLongitude()); // 使用经纬度信息创建一个LatLng对象。
            LatLngBounds.Builder builder = new LatLngBounds.Builder(); // 将latLng对象添加到LatLngBounds.Builder中，这通常用于地图上定义一个区域的边界。
